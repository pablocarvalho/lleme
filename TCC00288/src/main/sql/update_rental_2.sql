DROP FUNCTION IF EXISTS update_rental_2();
CREATE OR REPLACE FUNCTION update_rental_2(
    p_rental_id integer,
    ...
    RETURNS void AS '
DECLARE
    c_rental CURSOR (key integer) FOR
        SELECT *
        FROM rental
        WHERE rental_id = key
        FOR UPDATE OF rental;
    c_rental2 CURSOR (key1 smallint, key2 integer, key3 timestamp without time zone) FOR
        SELECT rental_date AS target_date
        FROM rental
        WHERE customer_id = key1
              AND rental_id <> key2
              AND rental_date > key3
        FOR SHARE OF rental;
...
BEGIN
    ...
    FOR r IN c_rental2(p_customer_id,p_rental_id,p_rental_date) LOOP
        IF EXISTS (WITH
                        rt AS (SELECT
                        ...
                        t2 AS (SELECT
                               ...
                               WHERE t21.customer_id = p_customer_id
                                     AND t21.rental_id <> p_rental_id
                                     AND t21.rental_date >= r.target_date - $$6 months$$::INTERVAL
                        ...
                        t3 AS (SELECT
                               ...
                               WHERE rt.customer_id = p_customer_id
                                     ...
                                     AND rt.rental_date >= r.target_date - $$6 months$$::INTERVAL)
                   SELECT SUM(t4.delay) AS delay
