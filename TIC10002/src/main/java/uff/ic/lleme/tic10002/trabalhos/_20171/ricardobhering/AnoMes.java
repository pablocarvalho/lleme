...
public class AnoMes {
	...
	private NoAnoMes inserir (int ano_mes, int cod_funcionario, float venda, NoAnoMes no) {
		if( no == null ) no = new NoAnoMes( ano_mes, cod_funcionario, venda, null, null );
		else if( ano_mes<no.chave ) no.esquerda = inserir( ano_mes, cod_funcionario, venda, no.esquerda );
		else if( ano_mes>no.chave ) no.direita = inserir( ano_mes, cod_funcionario, venda, no.direita );
	...
	private float vendasPorIntervaloAno_mes(NoAnoMes no, int ano_mes1, int ano_mes2){
		...
		if (no.chave>=ano_mes1) soma += vendasPorIntervaloAno_mes(no.esquerda, ano_mes1, ano_mes2);
		if (no.chave<=ano_mes2) soma += vendasPorIntervaloAno_mes(no.direita, ano_mes1, ano_mes2);
