...
public class Filiais {
	...
	private NoFilial inserir (int filial, int ano_mes, int cod_funcionario, float venda, NoFilial no) {
		...
		if( no == null ) no = new NoFilial( filial, ano_mes, cod_funcionario, venda, null, null );
		else if( filial<no.chave ) no.esquerda = inserir( filial, ano_mes, cod_funcionario, venda, no.esquerda );
		else if( filial>no.chave ) no.direita = inserir( filial, ano_mes, cod_funcionario, venda, no.direita );
		
		else if( no.chave == filial){

			no.arvore_ano_mes.inserir(ano_mes, cod_funcionario, venda);
		} //Finalizada a inser��o testa o balanceamento da �rvore.
		no = balanceia (no);
		return no;
	...
	private float vendasPorIntervaloFilial(NoFilial no, int filial1, int filial2){
		...
		if (no.chave>=filial1) totalzudo += vendasPorIntervaloFilial(no.esquerda, filial1, filial2);
		if (no.chave<=filial2) totalzudo += vendasPorIntervaloFilial(no.direita, filial1, filial2);
	...
	private float vendasPorDataFilial(NoFilial no, int filial1, int filial2, int data1, int data2){
		...
		if (no.chave>=filial1) totalzudo += vendasPorDataFilial(no.esquerda, filial1, filial2, data1, data2);
		if (no.chave<=filial2) totalzudo += vendasPorDataFilial(no.direita, filial1, filial2, data1, data2);
		if (no == this.raiz) System.out.println("\nTotal geral de vendas: "+totalzudo);
		return totalzudo;
	}
