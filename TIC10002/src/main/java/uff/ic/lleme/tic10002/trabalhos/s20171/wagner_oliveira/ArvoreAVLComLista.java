...
public class ArvoreAVLComLista<K extends Comparable, Entidade extends TemChave> {
    ...
    private NoLista inserir(NoLista<K, Entidade> no, K rotulo, Entidade objeto) {
        if (no == null) {
            ...
            no.esquerda = inserir(no.esquerda, rotulo, objeto); //System.out.println("esquerda " + rotulo + " " + no.chave);
        else if (rotulo.compareTo(no.chave) > 0)
            no.direita = inserir(no.direita, rotulo, objeto); //System.out.println("direita " + rotulo + " " + no.chave);
