// QuickSortDaniel.cpp : Defines the entry point for the console application.
//
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
...
/* quicksort */
List* int32_filter(int(*op)(int32_t a, int32_t b), int32_t pivot, List *ls)
{
	int32_t *n;
	...
	if (op(*n, pivot))
		return cons(first(ls), sizeof(int32_t),
			int32_filter(op, pivot, rest(ls)));
	else
		return int32_filter(op, pivot, rest(ls));
}

int int32_op_smaller(int32_t a, int32_t b) { return a <= b; }
int int32_op_larger(int32_t a, int32_t b) { return a > b; }

List* int32_smaller(int32_t pivot, List *ls)
{
	return int32_filter(int32_op_smaller, pivot, ls);
}

List* int32_larger(int32_t pivot, List *ls)
{
	return int32_filter(int32_op_larger, pivot, ls);
}
...
List* int32_qsort(List *ls)
{
	...
	return concat(int32_qsort(int32_smaller(*pivot, rest(ls))),
		cons(pivot, sizeof(int32_t), EMPTY),
		int32_qsort(int32_larger(*pivot, rest(ls))));
}
